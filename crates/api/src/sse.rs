...
pub fn parse_frame(frame: &str) -> Result<Option<StreamEvent>, ApiError> {
    ...
    if let Ok(error) = serde_json::from_str::<StreamErrorEnvelope>(&payload) {
        if error.kind == "error" {
...
mod tests {
    ...
    #[test]
    fn surfaces_stream_error_envelopes() {
        let frame = concat!(
            "event: error\n",
            "data: {\"type\":\"error\",\"error\":{\"type\":\"invalid_request_error\",\"message\":\"bad provider\"}}\n\n"
        );

        let error = parse_frame(frame).expect_err("stream error should surface");
        assert!(matches!(
            error,
            ApiError::StreamApi {
                ...
                ..
            } if error_type == "invalid_request_error" && message == "bad provider"
        ));
    }
}
